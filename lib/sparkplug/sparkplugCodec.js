...
function decodeMetric(raw, rawLong) {
  ...
  if (raw.isNull) {
    return { name: raw.name, alias: raw.alias, type: typeName, value: null, isNull: true, timestamp: raw.timestamp };
  }
  var value;
  switch (typeName) {
    case "Int8": case "Int16": case "Int32": case "UInt8": case "UInt16": case "UInt32":
  ...
  return { name: raw.name, alias: raw.alias, type: typeName, value: value, timestamp: raw.timestamp };
}

...
(function () {
    ...
    function reapplyInterpolationForInstance(screen, namespacedInstanceId, paramState) {
        screen.components.forEach(function (comp) {
            ...
            if (comp.type === "@lit-component") {
                renderLitComponentInstance(el, comp, interpolateProps(comp.props || {}, scope), makeCtx(screen, comp));
                return;
            }
            var typeDef = window.NEXA && window.NEXA.getComponent(comp.type);
            if (!typeDef || typeof typeDef.render !== "function") return;
            try {
                typeDef.render(el, interpolateProps(comp.props || {}, scope), makeCtx(screen, comp));
            } catch (e) {
                el.textContent = "(render error: " + e.message + ")";
            }
        });
    ...
    // Backs the "set-variable" Logic node. node.scope: the id of the container
    // that declares the variable, "" = the surface (the screen — or, for a node
    // that came from a template, that instance's own root scope). The value is
    // set on THAT scope, so everything inside it that doesn't shadow the name
    // sees it: those components re-render, and a nested instance whose
    // paramValues bind to it gets the new value passed in.
    function setVariable(screen, node, value) {
        var i = node.id.lastIndexOf("::");
        var instanceNs = i === -1 ? "" : node.id.slice(0, i);
        var scopes = screen.__scopes || {};
        var scope = node.scope
            ? scopes[instanceNs ? instanceNs + "::" + node.scope : node.scope]
            : (instanceNs ? screen.__paramStates && screen.__paramStates[instanceNs] : scopes[""]);
        if (!scope || typeof node.name !== "string" || !node.name) { logicTrace("set-variable: no such scope / name", node.scope, node.name); return; }
    ...
    function runLogicGraph(screen, node, msg, budget) {
        ...
        if (node.type === "function") {
            ...
            setVariable(screen, node, node.valueSource === "static" ? node.value : (msg && msg.payload));
    ...
    function mountAndFlatten(parentEl, comp, inheritedVis, templates, namespace, visitedTemplateIds, effectiveScreen, paramState, beforeEl) {
        ...
        if (isContainerNode(comp)) {
            ...
            el.__nexaNode = comp; // its children read its layout (applyNodeBox)
            (comp.children || []).forEach(function (child) {
                mountAndFlatten(el, child, vis, templates, childNamespace(namespace, child), visitedTemplateIds, effectiveScreen, innerScope);
            });
            return;
        }
    ...
    function mountScreen(screen, templates) {
        ...
        // the screen's variables: the root of every node's scope chain ("" = the screen)
        effectiveScreen.__scopes = { "": makeScope(null, screen.variables) };

...
var NexaModel = (() => {
  ...
  function contentOrigin(surface, id) {
  ...
  function surfaceScope(surface, isTemplate) {
    var scope = makeScope(null, surface && surface.variables);
    if (isTemplate) {
      (surface.params || []).forEach(function(p) {
        if (p && NAME_RE.test(p.name || "")) scope[p.name] = clone(p.defaultValue);
      });
    }
    return scope;
  }
  function visibleVariables(surface, ancestors2, isTemplate, self) {
    var seen = {}, out = [];
    function add(list, owner, valueKey) {
      (list || []).forEach(function(v) {
        if (!v || !NAME_RE.test(v.name || "") || seen[v.name]) return;
        seen[v.name] = true;
        out.push({ name: v.name, type: v.type || "string", value: v[valueKey], owner });
      });
    }
    var chain = (ancestors2 || []).slice();
    if (self && hasVariables(self)) chain.push(self);
  ...
  function allDeclarations(surface, walk2) {
    var out = [];
    (surface.variables || []).forEach(function(v) {
      out.push({ scopeId: "", scopeName: "Screen", variable: v });
    });
    walk2(surface, function(node) {
      if (hasVariables(node)) node.variables.forEach(function(v) {
        out.push({ scopeId: node.id, scopeName: node.name || node.type, variable: v });
      });
    });
    return out;
  }
